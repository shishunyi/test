var app = new Vue({
    ...
    methods:{
        ...
        {
            var that = this;
            layer.prompt({title: '请输入新的游戏ID', formType: 0}, function(pass, index){
                layer.close(index);
                that.loads();
                $.ajax({
                    url:'/savegameid',
        ...
        {
            var that = this;
            layer.prompt({title: '请输入新的密码', formType: 0}, function(pass, index){
                layer.close(index);
                that.loads();
                $.ajax({
                    url:'/savepassword',
        ...
        setroot:function(id){
            ...
            layui.use('laydate', function(){
                ...
                //执行一个laydate实例
                laydate.render({
                    elem: '.layui-layer-page #test1' //指定元素
                });
            });

            $('.layui-layer-page #rootid').val(id);
        },
